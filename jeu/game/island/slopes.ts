...
/**
 * Le renderer, pour lire et ecrire des pixels sans passer par un canvas 2D.
 *
 * Posé par l'application au demarrage (web ET natif). Pixi sait faire les
 * deux operations dont ce fichier a besoin — `getPixels` est un readPixels
 * GL, `BufferImageSource` une texture depuis un TypedArray — donc l'aller-
 * retour par un `<canvas>` qui existait ici n'apportait rien, et il rendait
 * ce module inutilisable hors navigateur (React Native n'a pas de DOM).
 */
let renderer: Renderer | null = null;

/** A appeler une fois le renderer cree, avant toute rampe. */
export function setSlopeRenderer(r: Renderer): void {
  renderer = r;
}

function exigerRenderer(): Renderer {
  if (!renderer) {
    throw new Error(
      '[slopes] setSlopeRenderer() n\'a pas ete appele : les rampes ont besoin '
      + 'du renderer pour lire et ecrire leurs pixels.',
    );
  }
  return renderer;
}
...
export function readCell(texture: Texture): ImageData {
  const { pixels, width, height } = exigerRenderer().texture.getPixels(texture);
...
export function textureFrom(pixels: ImageData, like: Texture): Texture {
  return new Texture({
    source: new BufferImageSource({
      resource: new Uint8Array(pixels.data),
      width: pixels.width,
      height: pixels.height,
      scaleMode: like.source.scaleMode,
      // Pas d'alphaMode explicite : le defaut de TextureSource est
      // 'premultiply-alpha-on-upload', exactement ce que le CanvasSource
      // d'avant heritait. Le rendu est donc inchange.
    }),
  });
}
